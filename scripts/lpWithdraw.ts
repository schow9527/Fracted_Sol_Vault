/*
LP withdraw: user withdraws liquidity back to own ATA for a mint.

Usage:
  ts-node scripts/lpWithdraw.ts \
    --program <PROGRAM_ID> \
    --mint <MINT_PUBKEY> \
    --amount <u64> \
    [--user <USER_PUBKEY>] \
...
async function main() {
  ...
  const sig = await program.methods
    .lpWithdraw(new anchor.BN(amount.toString()))
    .accounts({
      config: configPda,
      user: userPubkey,
      ...
      userDestinationToken: userAta,
      mint,
      tokenProgram,
    })
    .rpc();

  console.log('lp_withdraw tx:', sig);
}
