...
Usage:
  ts-node scripts/transferOut.ts \
    --program <PROGRAM_ID> \
    --mint <MINT_PUBKEY> \
    --recipient <RECIPIENT_PUBKEY> \
    --amount <u64> \
    [--authority <ADMIN_OR_ALLOWED_CALLER_KEYPAIR>] \
...
type Args = {
  ...
  recipient: string;
  amount: string;
  authority?: string; // path to keypair json
  rpc?: string;
  payer?: string;
  createAtas?: boolean;
};
...
function parseArgs(): Args {
  ...
  for (let i = 0; i < argv.length; i++) {
    ...
    if (a === '--recipient') args.recipient = v;
    if (a === '--amount') args.amount = v;
    if (a === '--authority') args.authority = v;
    if (a === '--rpc') args.rpc = v;
...
async function main() {
  ...
  // authority (admin or allowed caller) signer
  const authority = args.authority ? loadKeypair(args.authority) : payer;
  ...
  if (!recipientAtaInfo && args.createAtas) {
    ixes.push(createAssociatedTokenAccountInstruction(wallet.publicKey, recipientAta, recipient, mint, tokenProgram));
  }
  if (ixes.length) {
  ...
  const sig = await program.methods
    .transferOut(new anchor.BN(amount.toString()))
    .accounts({
      config: configPda,
  ...
  console.log('transfer_out tx:', sig);
}
