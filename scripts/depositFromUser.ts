/*
Deposit tokens from a user into the vault.

Usage:
  ts-node scripts/depositFromUser.ts \
    --program <PROGRAM_ID> \
    --mint <MINT_PUBKEY> \
    --amount <u64> \
    [--user <USER_PUBKEY>] \
...
async function main() {
  ...
  if (!userAtaInfo && args.createAtas) {
    ixes.push(createAssociatedTokenAccountInstruction(wallet.publicKey, userAta, userPubkey, mint, tokenProgram));
  }
  const vaultAtaInfo = await connection.getAccountInfo(vaultAta);
  if (!vaultAtaInfo && args.createAtas) {
    ixes.push(createAssociatedTokenAccountInstruction(wallet.publicKey, vaultAta, vaultAuthority, mint, tokenProgram));
  }
  if (ixes.length) {
  ...
  console.log('deposit_from_user tx:', sig);
}
